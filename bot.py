...
Стиль общения:
- Пиши развёрнуто, как в обычной переписке (1-2 предложения)
...
TASK_SUFFIX = "\n\nОтветь на последнее сообщение. Будь живой и интересной — развей тему, пошути или задай вопрос. Не отвечай односложно."
...
class Bot(commands.Bot):
    ...
    async def check_silence_loop(self):
        await self.wait_for_ready()
        silence_prompts = [
            "Задай интересный вопрос чату про игры, фильмы или что-то актуальное. Без приветствий, сразу к делу.",
            "Скажи что-то смешное или поделись мыслью чтобы разговорить чат. Можно задать вопрос.",
            "Напиши что-нибудь интересное чтобы начать общение. Поделись историей или задай вопрос.",
        ]
        while True:
