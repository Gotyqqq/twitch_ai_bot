...
class Bot(commands.Bot):
    ...
    def clean_response(self, text: str, state: ChannelState) -> str:
        ...
        for i, word in enumerate(words):
            ...
            if clean_word in COMMON_TWITCH_EMOTES or clean_word in state.all_known_emotes:
                cleaned_words.append(clean_word)
            # Проверяем паттерн смайлика (CamelCase или UPPERCASE)
            elif re.match(r'^[A-Z][a-z]*([A-Z][a-z]*)+$|^[A-Z]{3,}$', clean_word):
                # Похоже на смайлик - оставляем
                cleaned_words.append(clean_word)
            else:
